...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Don't reset the ref here - keep it true for the session
    };
  }, [user]);
...
{hasAccess && <div className="mb-6 rounded-2xl overflow-hidden border border-primary/30 shadow-lg shadow-primary/20 bg-background">
      ...
      <Dialog open={!!linkChoicePlatform} onOpenChange={(open) => !open && setLinkChoicePlatform(null)}>
        <DialogContent className="bg-card border-border w-[calc(100vw-2rem)] max-w-md overflow-x-hidden">
          <DialogHeader>
                ...
                <div className="space-y-3">
                  {urls.map((url, idx) => (
                    <div key={idx} className="space-y-1 min-w-0">
                      ...
                      <div
                        className="flex items-center gap-2 bg-background/50 border border-border rounded-md px-3 py-2 cursor-pointer hover:border-primary/50 transition-colors min-w-0"
                        onClick={() => {
