...
-- Convert existing access_days values from days to hours
UPDATE public.invites SET access_days = access_days * 24 WHERE access_days > 0;

-- Update the use_invite function to use hours instead of days
...
BEGIN
  ...
  -- Use hours instead of days for access duration
  UPDATE public.profiles 
  SET 
    has_access = true,
    access_expires_at = now() + (v_invite.access_days || ' hours')::interval,
    updated_at = now()
  ...
  RETURN jsonb_build_object('success', true, 'access_hours', v_invite.access_days);
END;
$$;
